...
const encodeUint = (value, bits) => {
    try {
        return internal_1.utils.toNBytes(typeof value === "number"
            ? value
        ...
        throw errors_1.ErrorWithCode.updateError(error, error.code || errors_1.RenJSError.INTERNAL_ERROR, `Unable to encode uint${bits} '${String(value)}'`);
    }
};
...
const withLength = (value) => internal_1.utils.concat([(0, exports.encodeU32)(value.length), value]);
exports.withLength = withLength;
/**
 * Encode a string, prefixed by its length.
 */
const encodeString = (value) => (0, exports.withLength)(internal_1.utils.fromUTF8String(value));
...
const encodePackStructType = (type) => {
    ...
    return internal_1.utils.concat([
 ...
 */
const encodePackListType = (type) => internal_1.utils.concat([
    new Uint8Array([(0, exports.encodePackType)(type)]),
...
const encodePackTypeDefinition = (type) => {
    if ((0, common_1.isPackStructType)(type)) {
        return (0, exports.encodePackStructType)(type);
    }
    else if ((0, common_1.isPackListType)(type)) {
        ...
        case types_1.PackPrimitive.Bytes: {
            return (0, exports.withLength)(value instanceof Uint8Array
                ...
                : // Supports base64 url format
                    internal_1.utils.fromBase64(value));
        }
        case types_1.PackPrimitive.Bytes32:
        case types_1.PackPrimitive.Bytes65:
 ...
 * pack's fields.
 */
const encodePackStruct = (type, value) => internal_1.utils.concat(type.struct.map((member) => {
    ...
    try {
        ...
        throw errors_1.ErrorWithCode.updateError(error, error.code || errors_1.RenJSError.INTERNAL_ERROR, `Unable to encode struct field ${key}`);
    }
}));
...
const encodeListStruct = (type, value) => {
    ...
    return internal_1.utils.concat(value.map((element, i) => {
        try {
...
const encodeTypedPackValue = ({ t, v }) => {
    try {
        ...
        return internal_1.utils.concat([encodedType, encodedValue]);
    }
    catch (error) {
        if (error instanceof Error) {
        ...
        throw new Error(`Error encoding typed pack value: ${internal_1.utils.extractError(error)}`);
    }
};
