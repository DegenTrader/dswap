...
import { PackPrimitive, } from "./types";
// === Pack Types ==============================================================
/**
 ...
 */
export const encodePackType = (type) => {
    // Primitive types.
    switch (type) {
        case "nil":
            return 0;
 ...
 */
export const encodeUint = (value, bits) => {
    try {
        return utils.toNBytes(typeof value === "number"
            ? value
        ...
        throw ErrorWithCode.updateError(error, error.code || RenJSError.INTERNAL_ERROR, `Unable to encode uint${bits} '${String(value)}'`);
    }
};
export const encodeU = (bits) => (value) => encodeUint(value, bits);
...
export const withLength = (value) => utils.concat([encodeU32(value.length), value]);
/**
 * Encode a string, prefixed by its length.
 */
export const encodeString = (value) => withLength(utils.fromUTF8String(value));
/**
 ...
 */
export const encodePackStructType = (type) => {
 ...
 */
export const encodePackListType = (type) => utils.concat([
    new Uint8Array([encodePackType(type)]),
 ...
 */
export const encodePackTypeDefinition = (type) => {
 ...
 */
export const encodePackPrimitive = (type, 
...
value) => {
    switch (type) {
        ...
        // Bytes
        case PackPrimitive.Bytes: {
            return withLength(value instanceof Uint8Array
                ? value
 ...
 * pack's fields.
 */
export const encodePackStruct = (type, value) => utils.concat(type.struct.map((member) => {
    ...
    try {
        ...
        throw ErrorWithCode.updateError(error, error.code || RenJSError.INTERNAL_ERROR, `Unable to encode struct field ${key}`);
    }
}));
/**
 ...
 * list's values.
 */
export const encodeListStruct = (type, value) => {
    const subtype = type.list;
    return utils.concat(value.map((element, i) => {
        try {
 ...
 * pack type.
 */
export const encodePackValue = (type, 
// eslint-disable-next-line @typescript-eslint/no-explicit-any
value) => {
    if (isPackStructType(type)) {
 ...
 */
export const encodeTypedPackValue = ({ t, v }) => {
    try {
        ...
        throw new Error(`Error encoding typed pack value: ${utils.extractError(error)}`);
    }
};
//# sourceMappingURL=binaryMarshal.js.map