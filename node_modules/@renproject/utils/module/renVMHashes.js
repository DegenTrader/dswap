...
import { pack } from "./libraries/pack";
/**
...
export const generateSHash = (selector) => {
    utils.assertType("string", { selector });
...
export const generateGHash = (pHash, sHash, to, nonce) => {
    utils.assertType("Uint8Array", { pHash, nonce, sHash, to });
...
export const generateNHash = (nonce, txid, txindex) => {
    utils.assertType("Uint8Array", { nonce, txid });
    utils.assertType("string", { txindex });
...
export const generateSighash = (pHash, amount, to, sHash, nHash) => {
    utils.assertType("Uint8Array", { pHash, nHash, sHash, to });
    utils.assertType("BigNumber", { amount });
    if (pHash.length !== 32) {
...
export const generateTransactionHash = (version, selector, packValue) => {
    utils.assertType("string", { version, selector });
    return utils.sha256(pack.binaryMarshal.encodeString(version), pack.binaryMarshal.encodeString(selector), pack.binaryMarshal.encodeTypedPackValue(packValue));
};
